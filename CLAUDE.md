...
2.  The exact project structure defined in `RULES.md`:
    ```
    tgbot_tcf/
      modules/
      handlers/
      ...
      keepalive.py
    ```
3.  A `requirements.txt` file generated from the existing `pyproject.toml` (e.g., via `uv export --format requirements-txt`). It must contain:
    * `python-telegram-bot[job-queue]==22.5`
    * `motor>=3.7.1`
    * `flask>=3.1.3`
    * `python-dotenv`
    (Pin versions according to `uv.lock`)
4.  A `.replit` file (same as `REPLIT.md`):
    ```
...
5.  A `c.env.example` file (example environment file) containing the placeholder keys:
    ...
    ```
6.  A `.gitignore` file with at least:
    ```
...
* All log messages must include the exact branding line: `𝘛𝘊𝘍 - 𝘛𝘳𝘢𝘯𝘴𝘴𝘪𝘰𝘯 𝘊𝘰𝘳𝘦 𝘍𝘦𝘥𝘦𝘳𝘢𝘵𝘪𝘰𝘯`
* All timestamps must be in UTC and formatted as `DD-MM-YYYY | HH:MM`.
...
* Use an **Opus style** of thinking: deep, analytical, step-by-step reasoning. When solving problems or designing logic, break down complex tasks into clear, structured, and well-justified steps. Prioritize correctness, robustness, and clarity in every decision.