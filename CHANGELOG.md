...
- **Workflow size limits** (`tcbot/modules/helper/workflows/`): extracted helper functions to keep all conversation workflows under 200 lines. `_execute_ban` (331 lines) split into `_execute_ban_update` and `_execute_new_ban`. `execute_warn` (304 lines) extracted auto-ban logic into `_execute_warn_auto_ban`. `build_modaction_conv` (298 lines) converted to `_ModActionFlow` class with named handler methods. `on_decision` in `appeal_flow.py` (215 lines) split into `_approve_appeal` and `_reject_appeal`.
- **Type safety** (`tcbot/modules/helper/workflows/ban_flow.py`, `appeal_flow.py`): changed `dict[str, object]` ban parameter types to `BanDoc` TypedDict for stronger static analysis. Added explicit `int()` casts for MongoDB numeric fields and `to_utc()` normalization for datetime values from database documents.
...
- **Bug #469** (`tcbot/database/groups_db.py`, `tcbot/database/cache.py`): `active_groups()` fetch query lacked `{"_id": 0}` projection, so the MongoDB `ObjectId` (`_id` field) was included in returned documents. `TwoLevelCache.get_or_fetch()` then called `json.dumps(val)` which raised `Object of type ObjectId is not JSON serializable`, silently degrading Redis L2 caching for the active-groups list on every startup cache warm-up. Initial fix added `{"_id": 0}` to the `.find()` projection in `groups_db.active_groups()`. A second serialization failure then surfaced: `GroupDoc.added_date` is a `datetime` object which `json.dumps` also cannot encode. Root-cause: the Redis write path in `TwoLevelCache` used the plain `json.JSONEncoder` with no extension for MongoDB-native types. Fixed by adding `_MongoJSONEncoder(json.JSONEncoder)` to `cache.py` that handles `datetime` → ISO-8601 string and falls back to `str()` for any other unknown type (defensive against future `ObjectId` leakage). Both `json.dumps` call sites in `TwoLevelCache` (`get_or_fetch` and `_redis_put_background`) now pass `cls=_MongoJSONEncoder`. Cache warm-up completes without any Redis serialization error. Ruff: clean. Import: OK.

- **`post_init` / `post_shutdown` not called in webhook mode** (`tcbot/__main__.py`): PTB v22.x `Application.initialize()` (called by `async with app:` `__aenter__`) explicitly does *not* invoke `post_init`; likewise `Application.shutdown()` (called by `__aexit__`) does not invoke `post_shutdown`. Both callbacks are only triggered by `run_polling()` and `run_webhook()`. In webhook mode, `_run_webhook_mode()` used `async with app:` + `app.start()` directly, so `_post_init` (MongoDB connect, indexes, owner seed, Redis, APScheduler, error reporter, cache warm-up) and `_post_shutdown` (Redis disconnect, scheduler stop) were never called. The health endpoint reported `mongodb=error`, `redis=error`, `scheduler=error`. Fixed by explicitly calling `await _post_init(app)` immediately after `async with app:` enters (before `app.start()`) and `await _post_shutdown(app)` in the `finally` block after `app.stop()`. Health endpoint now reports all subsystems `ok`. Ruff: clean. Import: OK.

## [Unreleased] - 2026-07-01 (session 177)

### Changed

- **Webhook transport mode** (`tcbot/__init__.py`, `tcbot/alive.py`, `tcbot/__main__.py`): Migrated bot from long-polling to native webhook transport per PROMPT.md v5.2.6. Environment auto-detection selects the transport at startup: Replit uses `REPLIT_DEV_DOMAIN` to build the webhook base URL; `WEBHOOK_URL` env var overrides; absence of either falls back to polling (local dev only, logged as WARNING). Added `webhook_url` and `webhook_secret` fields to `Configs`; `_auto_webhook_url()` implements the priority chain; `_resolve_webhook_secret()` reads `WEBHOOK_SECRET` env var or generates `secrets.token_hex(32)` at startup. `tcbot/alive.py` now imports `telegram.Update` at module level, adds `register_webhook(queue, loop, secret, bot)` to wire Flask to PTB's async update queue, and adds `POST /webhook` route that validates `X-Telegram-Bot-Api-Secret-Token`, calls `Update.de_json`, and uses `asyncio.run_coroutine_threadsafe` for thread-safe handoff. `tcbot/__main__.py` adds `_run_webhook_mode(app)` coroutine that calls `app.initialize()` (via `async with app:`), `app.start()`, `bot.set_webhook(url, secret_token, drop_pending_updates=True)`, verifies with `get_webhook_info()` (fails fast with CRITICAL log if URL mismatches), calls `register_webhook()`, waits for `SIGTERM`/`SIGINT`, then deletes webhook and calls `app.stop()` and `app.shutdown()`. `_build_application(polling=bool)` extracted to conditionally include `get_updates_connection_pool_size` only for polling mode. Long-polling fallback is preserved for local development with explicit WARNING log. `config.env.example` updated with `WEBHOOK_URL` and `WEBHOOK_SECRET` entries. Ruff: clean. Import: OK.
