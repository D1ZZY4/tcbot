...
- **Type safety** (`tcbot/`): hardened None handling across handler modules with guards for `effective_message`/`effective_chat`/`effective_user`, `.get()` fallbacks for TypedDict access, `cast()` expressions for `asyncio.gather` results with `return_exceptions=True`, and moved runtime-only imports into `TYPE_CHECKING` blocks in `groups.py`, `decorators.py`, and `prefixes.py`.
