...
- **Rolling restart when runtime files reach `main`** (`.github/workflows/run-bot.yml`, `docs/operations/ci-cd.md`, `README.md`): pushes touching bot code, translations, dependencies, or the runner itself now restart the live bot on the new commit within minutes instead of waiting for the window handover. Docs-only pushes never start a run. The new run cancels the stale one and waits for it to exit, so only one instance ever serves. Behavior changes: code pushes restart the bot; in-flight multi-step conversations start over.
