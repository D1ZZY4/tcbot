...
- **MTProto identity resolution beyond Bot API limits** (`tcbot/database/mtproto.py`, `tcbot/database/mtproto_store.py`, `tcbot/__init__.py`, `tcbot/__main__.py`, `tcbot/modules/helper/extraction.py`, `config.env.example`, `docs/getting-started/setup.md`, `tests/test_mtproto_base.py`, `tests/test_mtproto_store.py`): `API_ID` / `API_HASH` are mandatory and boot connects two clients sharing one MongoDB session store: the user session when it holds a login, plus a bot-token session that logs in fully automatically as the guaranteed floor. Silent-user lookups try one MTProto peer resolve before the multi-group sweep, with results flowing into the existing identity cache; an authorized legacy `<session>.session` file is imported once automatically. Transient runtime failures still degrade to None. Behavior changes: deployments without credentials no longer boot; user login stays optional thanks to the bot fallback.

- **MTProto session shared in MongoDB** (`tcbot/database/mtproto_store.py`, `tests/test_mtproto_store.py`): the login, peer hashes, and update states persist in the `mtproto_state` collection with the same semantics as Kurigram's file storage, so ephemeral runners and the beta host share one authorization. An authorized legacy `<session>.session` file is imported once automatically. Behavior changes: none for file-less deployments.
...
- **One-time MTProto session authorization** (`tcbot/database/mtproto_auth.py`, `docs/getting-started/setup.md`, `tests/test_mtproto_auth.py`): run the module with the account phone number, enter the login code (plus 2FA password when set), and the session lands in shared MongoDB storage ready for every instance. Wrong codes, expired codes, and bad passwords fail with a plain message and a nonzero exit. The CLI connects MongoDB first and normalizes bare numbers to E.164. Behavior changes: none.
