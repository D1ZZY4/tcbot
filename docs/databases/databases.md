...
| `cache.py` | in-process + Redis | `TTLCache[T]` (L1 in-process) and `TwoLevelCache[T]` (L1 in-process + L2 Redis). Four public singletons: `effective_role_cache`, `connected_cache`, `active_groups_cache`, `owner_id_cache`. Key methods: `get`/`put`/`invalidate` (sync, L1 only or L1+L2 background), `get_or_fetch` (async, L1 -> L2 -> DB, primary hot-path), `clear` (sync, L1 only; does **not** touch Redis), `clear_all` (async, L1 + Redis SCAN+UNLINK; use when the invalidation key is unknown). |
