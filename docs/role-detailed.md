...
This document describes the current role and staff-management behavior implemented by `tcbot/modules/admins.py`, `tcbot/modules/helper/workflows/promote_flow.py`, `tcbot/modules/helper/role_guard.py`, `tcbot/database/admins_db.py`, `tcbot/database/roles_db.py`, and `tcbot/database/queues_db.py`.
...
Effective roles are resolved by `roles_db.get_effective_role(user_id)`:
...
| Appeal decision buttons | Founder or Admin via `admins_db.is_staff` |
...
Only the Founder can use these callbacks. The handler checks `admins_db.is_owner(admin.id)` before reading the request.
...
The confirmation callback re-checks executor permission and target role before writing. If the target is Admin, it calls `admins_db.remove_admin(target_id)`. Otherwise, it calls `roles_db.remove_role(target_id)`.
...
4. `admins_db.set_owner(target_id)` replaces the single owner record in `tc_owners`.
...
`role_guard.auto_demote(...)` is used before ban/kick when the target has a lower staff role than the executor.
