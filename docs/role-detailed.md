...
Effective roles are resolved by `users_db.get_effective_role(user_id)`:
...
| Appeal decision buttons | Founder or Admin via `users_db.is_staff` |
...
2. If the target had a Developer/Tester role, `users_db.remove_role(target_id)` removes it from `tc_roles`.
...
Only the Founder can use these callbacks. The handler checks `users_db.is_owner(admin.id)` before reading the request.

### Approval

Approval does the following:

1. Adds the target to `tc_admins` with `users_db.add_admin(...)`.
...
The confirmation callback re-checks executor permission and target role before writing. If the target is Admin, it calls `users_db.remove_admin(target_id)`. Otherwise, it calls `users_db.remove_role(target_id)`.
...
4. `users_db.set_owner(target_id)` replaces the single owner record in `tc_owners`.
