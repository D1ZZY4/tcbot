...
| `appeal_approve_<ban_id>` / `appeal_reject:<ban_id>` | appeal review | ban ID tail | Staff verdict on a submitted appeal. Approve is underscore-delimited and Reject is colon-delimited as built by `appeal_review_kb()`; appeal IDs cannot contain the separator. Note: the review handler in `appeal_review_flow.py` registers and parses `appeal_reject_` (underscore), so its separator currently disagrees with the built button and must be reconciled. |
