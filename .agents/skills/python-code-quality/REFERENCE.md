...
For the parent skill instructions, see [`SKILL.md`](SKILL.md). For the
canonical project rules this reference enforces, see
[`../../rules/tooling-validation.md`](../../rules/tooling-validation.md),
[`../../rules/code-style.md`](../../rules/code-style.md), and
[`../../rules/comment-style.md`](../../rules/comment-style.md).
...
Ruff enforces a broad suite: pyflakes/syntax (`E`, `F`), import order (`I`), bugbear (`B`), comprehensions (`C4`), pydocstyle (`D`), boolean-trap (`FBT`), performance (`PERF`), misc (`PIE`), Pylint error/convention (`PLE`, `PLC`), pathlib (`PTH`), returns (`RET`), Ruff-specific (`RUF`), simplify (`SIM`), type-checking imports (`TC`), targeted try rules (`TRY400`, `TRY401`), pyupgrade (`UP`), and warnings (`W`). See `.agents/rules/tooling-validation.md` for validation commands and per-rule project policy. Apply remaining project conventions manually during review.
