...
- **Mute scope**: AI mute is network-wide across all connected groups. **Correction (superseding the original PRD claim)**: `muting_flow._execute_mute` cannot be called directly — it hard-depends on `update.effective_chat` and edits an interactive prompt message that doesn't exist in the AI path. AI mute uses new isolated `action_executor.execute_ai_mute()`/`execute_ai_mute_time()` that replicate the fan-out+DB-write tail without the interactive parts — same isolation pattern as ban/kick, not an exception to it.
...
- **Rules 09 (`fundraising`) and 16 (`nickname-pfp`)**: both promoted from human-only to `ai_enforceable: true` after review. `fundraising` follows normal text-based confidence/severity rules (not flag-only). `nickname-pfp` is enforced via a *separate* vision-capable model, not the text model — see `ai-moderation-vision.md` and `ai-moderation-reuse-audit.md`.
- **AI-enforceable rule count**: 23 of 27 (only `admin`, `admin-power`, `group-admin`, `group-ownership` remain human-only — an earlier PRD draft double-counted `group-ownership` as two separate rows, inflating the human-only count to 6).
