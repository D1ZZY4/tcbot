...
- Audit session 193 (2026-07-11): 1 doc bug fixed (Bug #502). 10 parallel sub-agents (SA1-SA10) across 3 waves covering all 75 Python files: zero code-level bugs found. Only 3 em-dashes in .md files (context.md, CHANGELOG.md, PLAN.md session-192 entries) corrected. Ruff: 75 files clean. Import: OK. Total bugs: #1-#502. Second consecutive session with no code bugs found. Audit approaching dry state.
...
- Audit session 190 (2026-07-11): 2 bugs fixed (Bug #492-#493), album dedup + double-submit guard. Total bugs: #1-#493.
...
| `tcbot/utils/` | Circuit breaker, logging, bounded fan-out dispatch, prefix filters, datetime helpers, error reporting. |
...
| 5 | `on_join_decision` sends false-positive success message when `complete_join` raises | `tcbot/modules/helper/workflows/connected_flow.py:494` | `complete_join` and `q.edit_message_text(self.connected_message(), ...)` run in the same `asyncio.gather(..., return_exceptions=True)`. `complete_join` now raises `RuntimeError` when `add_group` DB write fails. When it raises, `join_r` is a `BaseException` that is only logged; but `edit_message_text(connected_message())` was already dispatched in the same gather, so the group owner sees "This community is now connected" even though the group was never persisted to DB. The group receives no ban or mute enforcement, `/tcdisconnect` will fail to find it, and no corrective message is sent to the user. | Replace the combined gather with a sequential pattern: `await complete_join(...)` first; if it raises, edit the prompt to an error message and return. Only on success call `q.edit_message_text(connected_message())`. | `Resolved` |
| 6 | No re-appeal cooldown after rejection - `rejected_at` is stored but never read in `_start()` | `tcbot/modules/helper/workflows/appeal_flow.py:_start` | On rejection, `clear_review(ban_id)` clears `review_message_id`. `_start()` gates re-entry only on `if ban.get("review_message_id")` - since the field is now `None`, the user can re-appeal immediately after each rejection. `rejected_at` is stored in the ban document via `set_rejected_by` but `_start()` never checks it. A persistent user can spam-appeal immediately after every rejection with no enforced wait. | Add a `_REJECTION_COOLDOWN` (e.g. `timedelta(hours=24)`) and check `ban.get("rejected_at")` in `_start()`: if `utc_now() - to_utc(rejected_at) < _REJECTION_COOLDOWN`, reply with a cooldown message and return `END`. | `Resolved` |
| 7 | Warns cleared only in originating group on auto-ban - stale warns persist in other groups | `tcbot/modules/helper/workflows/warning_flow.py:276` | `db.warns_db.clear_warns(target_id, chat_id)` is called with only the originating `chat_id`. Warns the user accumulated in other groups are left intact in DB. After the user is unbanned, they re-enter those groups with existing warn counts (e.g. 2/3 in Group B). One further warn in Group B immediately re-triggers the auto-ban, which is surprising for both staff and the returning user. | On auto-ban, call `db.warns_db.clear_all_warns(target_id)` (a new `delete_many({user_id: target_id})` on `warn_counts`) instead of the group-scoped `clear_warns(target_id, chat_id)`. This ensures a clean slate across the federation on federation ban. | `Resolved` |

### P3 (Medium)

| # | Finding | Location (`file.py:line`) | Evidence (code quote / observed behavior) | Proposed Fix | Status |
|--|--|--|--|--|--|
| 1 | `uv run ruff` documented throughout `.agents/` but silently failed on Replit | `.agents/rules/STYLE-CODE.md:17`, `.agents/rules/RUFF.md:53` | `uv run ruff format .` exited with code 1 because ruff was in `[project.optional-dependencies.dev]`, which `uv run` does not install by default | Moved ruff to `[dependency-groups] dev = ["ruff"]` in `pyproject.toml`; `uv sync` now installs it automatically; `uv run ruff check .` and `uv run ruff format .` both pass clean | `Resolved` |
...
| 8 | `execute_unmute` fires federation-wide unrestrict without checking for an active mute | `tcbot/modules/helper/workflows/muting_flow.py:execute_unmute` | `execute_unmute` calls `fan_out([ctx.bot.restrict_chat_member(grp["chat_id"], target_id, permissions=full_perms) ...])` across all connected and primary groups with no prior check for `db.mutes_db.get_active_mute(target_id)`. Since `active_mutes` collection now exists (P5 #7), an active-mute guard is trivially available. When a staff member accidentally targets the wrong user or a user was never muted, the bot fires `restrict_chat_member` to all 50+ groups, writes an unmute log entry, and replies "restored in N/N groups" - a misleading success message for a no-op. Contrast with `execute_unban`, which calls `get_active_ban` first and returns early if no active ban exists. | At the top of `execute_unmute`, call `db.mutes_db.get_active_mute(target_id)`; if `None`, reply with "X has no active federation mute." and return early, mirroring the unban guard in `execute_unban`. | `Resolved` |
...
| 9 | `__module_name__` values follow no consistent naming convention | `tcbot/modules/admins.py:1`, `tcbot/modules/checking.py:1`, `tcbot/modules/maintenance.py:1`, and 12 others | Current names mix gerunds (`"Checking"`), bare verbs (`"Ban"`, `"Connect"`, `"Kick"`, `"Mute"`, `"Unban"`, `"Disconnect"`), plural nouns (`"Admins"`, `"Warnings"`, `"Groups"`), and abbreviations (`"Stats"`). Worst case: `maintenance.py` → `"Cleanup"` - the display name had no relation to the filename, the module slug, or the commands it exposes (`tcmaintenance` / `tcm`). These names render side-by-side in the help index menu. | Standardise to action nouns (title-cased, no trailing `s` unless inherently plural). Key corrections: `"Checking"` → `"Check"`, `"Cleanup"` → `"Maintenance"`, `"Admins"` → `"Admin"`. `_MODULE_NAME_MAP` in `help.py` is derived at import time from `__module_name__`, so no change needed there. All code changes landed in prior sessions; `docs/modules/modules.md` module table updated in session 173 to reflect the new `"Maintenance"` name. | `Resolved` |
...
| 11 | Anonymous group owner in anonymous mode receives unhelpful `/tcdisconnect` refusal with no guidance | `tcbot/modules/disconnecting.py:cmd_tcdisconnect` | `cmd_tcdisconnect` calls `get_chat_member(chat.id, user.id)` where `user.id == 1087968824` (GroupAnonymousBot) when the group owner has anonymous mode active. Telegram returns status "administrator" (not "creator") for GroupAnonymousBot, so `is_group_owner = member.status == "creator"` evaluates to `False`. The command replies "Only the group owner or TC admins can disconnect this group." with no indication that the cause is anonymous mode. The owner has no way to disconnect their own group without either disabling anonymous mode or asking a staff member to run `/rmtc`. | Amend the refusal message when `user.id == decorators._ANON_BOT_ID` to read: "Anonymous admin mode is active. Please send this command from your personal account, or ask TC Staff to run /rmtc." | `Resolved` |
| 12 | Federation staff in anonymous admin mode can approve/reject appeals via buttons but cannot use any command - undocumented behavioral inconsistency | `tcbot/modules/helper/decorators.py:_is_anon_admin`, `tcbot/modules/helper/workflows/appeal_flow.py:on_decision` | All auth decorators check `_is_anon_admin(update)` on the command message and return `_ERR_ANON_ADMIN` when `effective_user.id == 1087968824`. However, in `on_decision`, `update.effective_user` for a button press (callback query) always carries the presser's real Telegram user ID even when they are in anonymous admin mode - Telegram never substitutes GroupAnonymousBot for callback senders. So a staff member who is anonymous in the group CAN click Approve/Reject on the appeal card and their real `admin.id` passes `is_staff()`. The result: anonymous staff cannot `/tcban` or `/tcwarn` but can silently make appeal decisions. This is not documented anywhere and may surprise admins who assume anonymous mode fully restricts them. | Document the distinction in `docs/appeal-detailed.md` and in the help text for `appeals.py`: anonymous admin mode blocks commands but not button interactions. No code change required if the behavior is intentional. | `Resolved` |
...
| 8 | MongoDB circuit breaker singleton existed but was never wired into actual Motor calls | `tcbot/database/mongos.py:107`, `tcbot/utils/circuit_breaker.py:211` | `circuit_breaker.mongodb` singleton was created and exported in session 161, reported in `/health` under `circuit_mongodb`, and consumed by `alive.py` to flag `degraded` when open. However, no Motor coroutine ever called `_mongo_cb.call(...)`, `_mongo_cb.record_failure()`, or `_mongo_cb.record_success()`, so the circuit was permanently CLOSED regardless of actual MongoDB connectivity: no failure could trip it, and no probe could self-heal it. The health endpoint showed `circuit_mongodb: closed` even during a real MongoDB outage. | `mongos.py` now imports `mongodb as _mongo_cb` from `tcbot.utils.circuit_breaker`. `connect()` pings MongoDB via `await _mongo_cb.call(_db.command("ping"))`: a successful startup ping records CLOSED and repeated failures during recovery probes correctly advance the state machine. Added module-level `async def db_call(coro)` helper so database helper modules can route any Motor coroutine through the circuit breaker and fast-fail with `CircuitOpenError` instead of waiting the 45-second socket timeout when the cluster is unreachable. `docs/databases/databases.md` updated with `db_call()` entry. | `Resolved` |
